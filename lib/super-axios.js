...
var AxiosManager = /** @class */ (function () {
    // 初始化axios
    function AxiosManager(axiosConfig, ext) {
        var _this = this;
        this.cancel = null; // 取消请求回调方法
        this.delay = false; // 是否延时
        this.needCancel = false; // 是否需要取消
        this.shake = false; // 是否需要阻止
        this.context = {}; // 当前请求上下文
        if (ext) {
        ...
        this.Http.interceptors.request.use(function (config) { return __awaiter(_this, void 0, void 0, function () {
            var url, method, data, params, needCancel, that;
            return __generator(this, function (_a) {
                ...
                if (needCancel) {
                    config.cancelToken = new cancelToken(function (c) {
                        AxiosManager.cancelMap.set(that.getKey(url, method), {
                            url: url,
                            method: method,
                            data: data,
                            params: params,
                ...
                if (AxiosManager.request) {
                    return [2 /*return*/, AxiosManager.request(config)];
                }
            ...
            // 清除防抖
            if (shake) {
                AxiosManager.shakeMap.delete(_this.getKey(url, method));
            }
            // 清除可取消队列
            if (needCancel) {
                AxiosManager.cancelMap.delete(_this.getKey(url, method));
            }
        });
        // 相应拦截器
        this.Http.interceptors.response.use(function (res) {
            ...
            if (res.status === 200) { // 请求服务端成功
                ...
                if (AxiosManager.tryingMap.get(_this.getKey(url, method))) {
                    AxiosManager.tryingMap.delete(_this.getKey(url, method));
                    // 然后再看是否重连队列已被清空，清空后直接执行success方法
                    if (AxiosManager.tryingMap.size === 0) {
                        AxiosManager.autoAttempt = false;
            ...
            if (error.message.search('timeout') > -1) {
                var maxReconnectionTimes = AxiosManager.maxReconnectionTimes, tryingMap = AxiosManager.tryingMap;
                var _c = _this.context, url_1 = _c.url, method_1 = _c.method;
                var request = tryingMap.get(_this.getKey(url_1, method_1));
                if (request && request.autoCounts) {
                    if (maxReconnectionTimes > request.autoCounts) {
                        request.autoCounts++;
                        AxiosManager.tryingMap.set(_this.getKey(url_1, method_1), request);
                        return _this.reconnect();
                    }
                    else {
                        AxiosManager.tryingMap.delete(_this.getKey(url_1, method_1));
                        if (AxiosManager.tryingMap.size === 0) {
                            AxiosManager.autoAttempt = false;
                    ...
                    AxiosManager.tryingMap.set(_this.getKey(url_1, method_1), __assign(__assign({}, _this.context), { autoCounts: 1 }));
                    return _this.reconnect();
                }
            }
            return Promise.reject(error);
    ...
    AxiosManager.prototype.reconnect = function () {
        var _this = this;
        if (!AxiosManager.autoAttempt) {
            AxiosManager.autoAttempt = true;
        ...
        return Reconnection.then(function () {
            return _this.dispatch(_this.context);
        });
    };
    ...
    AxiosManager.prototype.dispatch = function (context) {
        return __awaiter(this, void 0, Promise, function () {
            var url, method, data, params, headers, delayTime, delay, shake, needCancel, shakeMap, cancelMap, it;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        // 保存当前请求上下文
                        ...
                        shakeMap = AxiosManager.shakeMap, cancelMap = AxiosManager.cancelMap;
                        // 是否需要取消上一次的请求
                        if (needCancel && cancelMap.has(this.getKey(url, method))) {
                            it = cancelMap.get(this.getKey(url, method));
                            if (it && it.cancel) {
                                it.cancel("取消接口");
                                AxiosManager.cancelMap.delete(this.getKey(url, method));
                            }
                        }
                        if (!delay) return [3 /*break*/, 2];
                        AxiosManager.Timer && clearTimeout(AxiosManager.Timer);
                        return [4 /*yield*/, this.sleep(delayTime || AxiosManager.delayTime)];
                    case 1:
                        _a.sent();
                        _a.label = 2;
                    case 2:
                        ...
                        if (shake && shakeMap.has(this.getKey(url, method))) {
                            return [2 /*return*/, Promise.reject("请勿重复提交")];
                        }
                        // 提交请求
                        return [2 /*return*/, this.Http({ url: url, method: method, data: data, params: params, headers: __assign(__assign({}, this.Http.defaults.headers), headers) })];
                }
            });
        });
    };
    ...
    AxiosManager.cancelMap = new Map(); // 请求队列
    AxiosManager.shakeMap = new Map(); // 防抖的请求队列
    AxiosManager.tryingMap = new Map(); // 重连队列
    AxiosManager.autoAttempt = false; // 自动尝试重连
    AxiosManager.timeStep = 1000; // 断线重连时间间隔
    AxiosManager.maxReconnectionTimes = 5; // 最大重连数
    AxiosManager.delayTime = 500; // 延时时间
    return AxiosManager;
}());
